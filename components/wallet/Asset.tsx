    ...
    return (
        <div className=" bg-base-100 shadow-xl w-80 flex flex-col flex-shrink-0 rounded-xl select-none " id={`${item?.unit}`}>
            <figure className="w-25 h-25 relative h-[300px]">
                {metadata ? (
                    <Image unoptimized={true} className={classNames("rounded-t-xl", locked !== undefined && 'grayscale')} src={`https://ipfs.blockfrost.dev/ipfs/${(metadata?.image || "").replace("ipfs://", "")}`} width={400} height={400} alt="Shoes"></Image>
                ) : (
